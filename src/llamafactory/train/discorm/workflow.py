...
def run_discorm(
    ...
    dataset_module = get_dataset(template, model_args, data_args, training_args, stage="discorm", **tokenizer_module)
    model = load_model(
        tokenizer, model_args, finetuning_args, training_args.do_train, 
        add_valuehead=True, is_disco=finetuning_args.disco
    )
